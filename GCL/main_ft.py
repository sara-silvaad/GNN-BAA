...
FT_MODEL_PATH, FT_RESULTS_PATH = solve_paths_pre(SAVED_MODELS_PATH_ft, RESULTS_PATH_ft, DATA_AUG, LAMBDA_VAL)
ft_model_name = f'{MODEL}_{DATA_AUG}_{BATCH_SIZE_ft}_{EPOCHS_ft}_{LR_ft}_{EARLY_STOPPING_PATIENCE_ft}'

# About the pretrained model
PRE_MODEL_PATH, _ = solve_paths_pre(SAVED_MODELS_PATH_pre, RESULTS_PATH_pre, DATA_AUG, LAMBDA_VAL)
...
for run in range(RUNS_ft):
    ...
    encoder = load_pretrained_model(f"{PRE_MODEL_PATH}/{pre_model_name}", HIDDEN_DIMS, dataset.num_classes, DATA_AUG)
    classifier = FineTuneClassifier(input_dim=sum(HIDDEN_DIMS[1:]), hidden_dim = 16)

    criterion_classif = torch.nn.CrossEntropyLoss(reduction= 'mean')

    # train and test
    result, best_model = train_and_evaluate_ft(encoder, classifier, train_loader, val_loader, EPOCHS_ft, LR_ft, criterion_classif, EARLY_STOPPING_PATIENCE_ft, EPOCHS_all_ft)
    test_result = test_ft(encoder, classifier, test_loader, criterion_classif)
    results.append(result)
    test_results.append(test_result)

    # save results and model
    create_plots_and_save_results(result, test_ft, f'{run}_{ft_model_name}', best_model, FT_RESULTS_PATH, FT_MODEL_PATH)
