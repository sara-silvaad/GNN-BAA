...
def get_file_names(r_num_all):
    ...
    for r_num in r_num_all:
        ...
        HCP_paths[r_num]['lr_msmall'] = f"rfMRI_REST{r_num}_LR_Atlas_MSMAll_hp2000_clean.dtseries.nii"
        HCP_paths[r_num]['rl_msmall'] = f"rfMRI_REST{r_num}_RL_Atlas_MSMAll_hp2000_clean.dtseries.nii"

        HCP_paths[r_num]['lr_no_msmall'] = f"rfMRI_REST{r_num}_LR_Atlas_hp2000_clean.dtseries.nii"
        HCP_paths[r_num]['rl_no_msmall'] = f"rfMRI_REST{r_num}_RL_Atlas_hp2000_clean.dtseries.nii"

        HCP_paths[r_num]['lr_msmall_hcp_rel_path'] = f"/MNINonLinear/Results/rfMRI_REST{r_num}_LR/" + HCP_paths[r_num]['lr_msmall']
        HCP_paths[r_num]['rl_msmall_hcp_rel_path'] = f"/MNINonLinear/Results/rfMRI_REST{r_num}_RL/" + HCP_paths[r_num]['rl_msmall']

        HCP_paths[r_num]['lr_no_msmall_hcp_rel_path'] = f"/MNINonLinear/Results/rfMRI_REST{r_num}_LR/" + HCP_paths[r_num]['lr_no_msmall']
        HCP_paths[r_num]['rl_no_msmall_hcp_rel_path'] = f"/MNINonLinear/Results/rfMRI_REST{r_num}_RL/" + HCP_paths[r_num]['rl_no_msmall']
...
def list_files(patient_id, HCP_paths, subject_dir, r_num_all):
    ...
    for r_num in r_num_all:
        #msall
        #LR
        hcp_path = HCP_paths['path2HCP_1200'] + patient_id + HCP_paths[r_num]['lr_msmall_hcp_rel_path']
        readable_name = "LR_msmall"
        local_path = subject_dir + "/" + HCP_paths[r_num]['lr_msmall']
        files.append({"hcp_path": hcp_path, "readable_name": readable_name, "local_path": local_path})
        #RL
        hcp_path = HCP_paths['path2HCP_1200'] + patient_id + HCP_paths[r_num]['rl_msmall_hcp_rel_path']
        readable_name = "RL_msmall"
        local_path = subject_dir + "/" + HCP_paths[r_num]['rl_msmall']
        files.append({"hcp_path": hcp_path, "readable_name": readable_name, "local_path": local_path})

        #no msall
        #LR
        hcp_path = HCP_paths['path2HCP_1200'] + patient_id + HCP_paths[r_num]['lr_no_msmall_hcp_rel_path']
        readable_name = "LR_no_msmall"
        local_path = subject_dir + "/" + HCP_paths[r_num]['lr_no_msmall']
        files.append({"hcp_path": hcp_path, "readable_name": readable_name, "local_path": local_path})
        #RL
        hcp_path = HCP_paths['path2HCP_1200'] + patient_id + HCP_paths[r_num]['rl_no_msmall_hcp_rel_path']
        readable_name = "RL_no_msmall"
        local_path = subject_dir + "/" + HCP_paths[r_num]['rl_no_msmall']
        files.append({"hcp_path": hcp_path, "readable_name": readable_name, "local_path": local_path})
...
def contstruct_time_series(subject, HCP_paths, subject_dir, roi_path, timeseries_dir, r_num_all):
    ...
    for f in list_files(subject, HCP_paths, subject_dir, r_num_all):
        ...
        if file_name in [ 
                        f'rfMRI_REST1_RL_Atlas_hp2000_clean.dtseries.nii',
                        f'rfMRI_REST1_LR_Atlas_hp2000_clean.dtseries.nii',
                        f'rfMRI_REST2_RL_Atlas_hp2000_clean.dtseries.nii',
                        f'rfMRI_REST2_LR_Atlas_hp2000_clean.dtseries.nii',
                        ]:
...
def process_files(subject, HCP_paths, subject_dir, roi_path, kind, r_num_all, corr_path, timeseries_dir):
    ...
    subj_timeseries = contstruct_time_series(subject, HCP_paths, subject_dir, roi_path, timeseries_dir = timeseries_dir, r_num_all = r_num_all)
    ...
    if kind == None:
        ...
        corr_matrix, cov_matrix = calculate_fc_matrix(concatenated_ts, kind, subject, corr_path)
...
delete_files = True # True or False, delete files once processed as theyre downloaded
